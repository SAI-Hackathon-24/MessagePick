...
> **（注：除当前仓库代码外，还需要前置wechat-cli，链接https://github.com/SAI-Hackathon-24/wechat-cli，并依照其中描述安装该包）
---
...
> 未配置时采集（数据导入）仍可用，但梗分析 / 信息提取 / 社交画像的模型任务会失败（错误可见，不伪造数据）。
> **分析按需触发**：在筛选条选中群后点「分析」即对所选群执行梗分析 + 信息提取（未选群 = 全部群）；
> 采集后的自动分析默认关闭（可在设置里开启「采集完成后自动分析全部群」）；社交画像在进入社交页时按需构建（跨群口径）。

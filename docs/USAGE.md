...
## 10. 演示版（仅界面预览，用静态演示数据）

除了上面这套接真实后端的应用，本机上还有一份**演示版前端**，用于**纯看界面**：

| 项 | 说明 |
| --- | --- |
| 位置 | `../webui/`（与仓库同级，即 `MessagePick/webui/`）——**不在本仓库内**，独立目录 |
| 数据 | 内置演示数据（`src/api/fixtures.ts` + `mock.ts`），**不连后端、不需要 API Key、不需要采集** |
| 端口 | 默认 <http://127.0.0.1:5273> |
| 用途 | 演示 / 截图 / 讲布局；**不代表真实数据结果** |
...
| | 演示版（5273） | 正式版（本仓库，如 8787） |
| --- | --- | --- |
| 数据来源 | 内置演示数据 | 真实采集 + 模型分析 |
| 顶部横幅 | 有「开发期数据模式」提示 | 无 |
| 写操作 | 本地模拟 | 需启动令牌（`#token=`） |
| 数字含义 | 演示值，可能出现夸张数值 | 真实统计 |

> ⚠️ 看到「周环比 +900719925474099100%」「活跃天数 0」这类**不可能的数值**时，
> 说明你打开的是演示版（`9007199254740991` 是 `Number.MAX_SAFE_INTEGER`，演示数据的典型症状）。
> 真实数据请用正式版地址。

...
| 涉及的契约 | 不产生 `API-###`（经阶段 3 裁定）；消费：`API-001`、`API-002`、`API-004` ~ `API-006`、`API-009` ~ `API-034`（`API-004` 的群清单读路径经 CHG-026 补齐）；无 `DM-###` 落点（经阶段 4 裁定） |
...
- 明确不做（口径来自上游）：不提供读取模块内部队列深度的队列状态接口，`detailed-design.md` §1.3 / §6.4 只允许展示外壳发起操作的排队与在途数量（CHG-026）；不提供演示数据 / 演示模式入口（`REQ-019`）；不新增任何第二组同类筛选控件（`REQ-004` / `REQ-049`）。
...
### 4.5 预热（HLD 决策 9）与操作登记（CHG-026 口径）
...
- **背景与约束**：`REQ-004` 要求唯一筛选源、三模块同步受限；`REQ-049` 禁止第二组控件；删除与筛选需要群清单读路径（`API-004` 的群读口径，CHG-026）。
...
- **背景与约束**：HLD 决策 9 要求采集完成后后台触发；CHG-026 收敛可见性——设置页只展示外壳发起操作的排队与在途数量，模块内部队列深度不对外暴露。

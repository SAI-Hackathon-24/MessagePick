...
| API-002 | 查询更新状态 | MOD-001 | MOD-004 | REQ-002、REQ-003、REQ-016 | 返回是否有数据、「记录更新至 X」与各来源状态 |
| API-003 | 写入记录 | MOD-002 | MOD-001、MOD-005 ~ MOD-008 | REQ-007、REQ-011 | 持久化原始记录与派生结果（按记录身份去重） |
| API-004 | 按条件读取 | MOD-002 | MOD-001、MOD-005 ~ MOD-008 | REQ-006、REQ-007 | 按实体类型 + 筛选条件分页读取记录 |
...
- **出参（成功）**：
  ...
  | 字段 | 类型 | 含义 |
  | --- | --- | --- |
  | 梗条带 | 列表 | 每梗一行：首现 / 峰值 / 沉寂点 / 活跃天数 / 按月强度（REQ-025、REQ-030） |
...
- **出参（成功）**：
  ...
  | 字段 | 类型 | 含义 |
  | --- | --- | --- |
  | 分组的通知列表 | 列表 | 每组含通知条目：来源、类型、优先级、待办状态、来源消息引用（REQ-045） |
